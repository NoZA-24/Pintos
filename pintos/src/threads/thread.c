...
/* If the current thread isn't idle, change the state of current to BLOCKED. */
void
...
/* Update next tick by minimum tick. */
void
...
/* Wake Thread which have its tick lower or equal to ticks.
   Also, update next tick. */
void
...
/* Test if current running thread has maximum priority.
   if not, yield it */
void
test_max_priority(void)
{
  if(!list_empty(&ready_list))
  {
    ...
    int rd_priority = list_entry(list_front(&ready_list),
                                  struct thread, elem)->priority;
    if(cur_priority < rd_priority)
      thread_yield();
  }
}
...
/* Donate current thread's priority to it's connected threads. */
void
...
/* Travel donation list and remove lock. */
void
...
/* Refresh all thread's priority in donation list. */
void
refresh_priority(void)
{
  ...
  while(e!=NULL)
  {
    e_priority = list_entry(e,struct thread,donation_elem)->priority;
    if (cur->priority < e_priority)
      cur->priority = e_priority;
    e=e->next;
  }
}

/* Calculate priority in 4.4BSD scheduler. */
void
...
/* Calculate recent_cpu in 4.4BSD scheduler. */
void
...
/* Calculate load_avg in 4.4BSD scheduler. */
...
/* Recalculate all thread's recent cpu and priority. */
void
mlfqs_recalc(void)
{
  struct list_elem *e;
...
/* Count number of thread which is not idle thread. */
int
count_thread(void)
{ 
  int cnt=0;
